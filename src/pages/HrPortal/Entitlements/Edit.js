...
const Edit = (props) => {
    ...
    const { isOpenEdit, handleCloseEdit, fetchLeaveEntitlementData, data } = props;
    ...
    const initialValues = {
        ...
        maximumPerMonth: data?.maximumPerMonth,
        maximumDaysAdvance: data?.maximumDaysAdvance,
        maximumDaysBackDate: data?.maximumDaysBackDate,
        minimumDaysRestriction: data?.minimumDaysRestriction,
        abbreviation: data?.abbreviation,
        canCarryForward: data?.canCarryForward,
        sandwiched: data?.sandwiched,
        paid: data?.paid,
        minimumLeavePeriod: data?.minimumLeavePeriod,
        balanceVisible: data?.balanceVisible,
        encachable: data?.encachable,
    ...
    const edit = async (values) => {
        const pyload = {
            ...
            canCarryForward: values?.canCarryForward,
            sandwiched: values?.sandwiched,
            paid: values?.paid,
            minimumLeavePeriod: values?.minimumLeavePeriod,
            balanceVisible: values?.balanceVisible,
            encachable: values?.encachable,
            isEarned: values?.isEarned,
            noOfDays: values?.noOfDays,
            period: values?.period,
            calculationBase: values?.calculationBase,
            modifiedOn: new Date()
        }
        const result = await apiput('/api/leaveEntitlement', pyload);
                ...
                <DialogContent dividers>
                    <form>
                        <Grid container rowSpacing={3} columnSpacing={{ xs: 0, sm: 5, md: 4 }}>
                            ...
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.canCarryForward || null}
                            ...
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.sandwiched || null} onChange={(e) => formik.setFieldValue('sandwiched', e.target.checked)} />} name='sandwiched' labelPlacement="start" label="Sandwiched" />
                            </Grid>

                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.paid || null} onChange={(e) => formik.setFieldValue('paid', e.target.checked)} />} name='paid' value={formik.values.paid} onChange={formik.handleChange} labelPlacement="start" label="Paid" />
                            </Grid>
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.minimumLeavePeriod || null} onChange={(e) => formik.setFieldValue('minimumLeavePeriod', e.target.checked)} />} name='minimumLeavePeriod' labelPlacement="start" label="Minimum Leave Period" />
                            </Grid>
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.balanceVisible || null} onChange={(e) => formik.setFieldValue('balanceVisible', e.target.checked)} />} name='balanceVisible' labelPlacement="start" label="Balance Visible" />
                            </Grid>
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.encachable || null} onChange={(e) => formik.setFieldValue('encachable', e.target.checked)} />} name='encachable' labelPlacement="start" label="Encachable" />
                            </Grid>
                            <Grid item xs={12} sm={12} md={12}>
                                <FormControlLabel control={<Checkbox checked={formik.values.isEarned || null} onChange={(e) => formik.setFieldValue('isEarned', e.target.checked)} />} name='isEarned' labelPlacement="start" label="Is Earned" />
                            </Grid>
                            {
