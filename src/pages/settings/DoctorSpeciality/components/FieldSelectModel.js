...
const FieldSelectModel = ({ open, close, fileData }) => {
    ...
    const fieldsInCrm = [
        { Header: 'Division Name', accessor: 'divisionName', width: 20 },
        { Header: 'Speciality Name', accessor: 'specialityName', width: 20 },
        ...
        onSubmit: (values, { resetForm }) => {
            const Data = importedFileData?.map((item, ind) => {
                return {
                    ...
                    specialityName: item[values.specialityName || "specialityName"] || '',
                    group: item[values.group || "group"] || '',
    ...
    const AddData = async (data) => {
        try {
            const response = await apipost('/api/doctorspeciality/addMany', data)
