...
const FieldSelectModel = ({ open, close, fileData }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            const Data = importedFileData?.map((item, ind) => {
                const holidayDate = moment(item[values.holidayDate || "holidayDate"]);
                return {
                    zone: item[values.zone || "zone"] || '',
                    holidayDate: holidayDate.isValid() ? item[values.holidayDate || "holidayDate"] || '' : '',
                    holidayName: item[values.holidayName || "holidayName"] || '',
    ...
    const AddData = async (data) => {
        try {
            const response = await apipost('/api/holidaycalendar/addMany', data)
